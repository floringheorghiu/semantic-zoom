...
| `scripts/build-layers.mjs` | Optional starting-point helper for large files (100+ blocks): groups blocks under `##` headings into sections, then classifies sections into meta groups by keyword-matching titles. Requires per-document customization (`META_GROUPS`, `classify()`) — not a generic zero-config tool. Ported from a Python helper an agent (MiMo Code) built ad hoc while tagging a large file in a sibling project. |
| `scripts/check-layers.mjs` | Zero-dependency structural self-check on a hand- or `build-layers.mjs`-authored `layers.json`, run before `assemble.mjs`: every section assigned to exactly one meta node, no unknown/duplicate keys, no empty paragraph lists. `assemble.mjs` enforces the same constraints regardless — this catches the mistake one step earlier. |
